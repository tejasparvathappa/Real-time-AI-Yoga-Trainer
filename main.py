...
def main():
    st.title("Real time virtual Yoga Trainer")
    frame_placeholder = st.empty()
    # Selection box for the yoga pose
    choice = st.selectbox(
        "Choose a yoga pose you would like to begin with:",
        ["None", "T Pose (Easy)", "Arms Pose (Easy)", "Mountain Pose (Intermediate)", "Tree Pose (Intermediate)", "Warrior Pose (Advanced)", "Kumara Pose (Advanced)"]
    )

    poses = {
        "T Pose (Easy)": "video/t.jpg",
        "Arms Pose (Easy)": "video/arms_pose.jpg",
        "Mountain Pose (Intermediate)": "video/mountain.jpg",
        "Tree Pose (Intermediate)": "video/tree.png",
        "Warrior Pose (Advanced)": "video/warrior2.jpg",
        "Kumara Pose (Advanced)": "video/kumara_pose.jpeg",
    }
    if st.button("Show Pose"):
        # Mapping the choice to your test1 function with the corresponding image paths

        if choice in poses:
            st.write(f"You selected {choice}.")
            st.image(poses[choice], caption=f"{choice} Pose")
        else:
            st.write("Invalid choice")

    if st.button("Start Webcam"):
        st.write("Please align yourself with the camera")
        cap = cv2.VideoCapture(0)  # Use the correct device index

        x = extractKeypoint(poses[choice])
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened():
                ...
                # Process the frame with pose detection and comparison logic
                # Note: This is a placeholder. You need to integrate your pose processing logic here.
                ...
                # Use a button to stop the webcam
